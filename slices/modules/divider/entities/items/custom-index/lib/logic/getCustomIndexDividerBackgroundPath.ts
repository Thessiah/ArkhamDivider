...
export type CustomIndexDividerBackgroundPathOptions = {
	width: number;
	height: number;
	cornerRadius: number;
...
type Options = CustomIndexDividerBackgroundPathOptions;
...
const getTabContentWidth = (options: {
	...
	return getCustomIndexDividerTabWidth(options) - 2 * tabSideWidth;
};

export const getCustomIndexDividerTabWidth = ({
	tabWidths,
	tabSize,
	width,
	tabWidthOffset = 0,
...
export const getCustomIndexDividerTabLeft = ({
	tabSize,
	tabIndex,
	tabsCount,
	width,
...
const getTabbedPath = (options: TabbedOptions) => {
	...
	const tabLeft = getCustomIndexDividerTabLeft(options);
 ...
 */
export const getCustomIndexDividerBackgroundPath = (
	options: Options,
): string => {
	const x = options.x ?? 0;
	const y = options.y ?? 0;
